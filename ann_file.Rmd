...
train_ann <- spotify_train
test_ann <- spotify_test

train_ann$target <- as.factor(train_ann$target)
test_ann$target <- as.factor(test_ann$target)
...
ann_predictions <- as.data.frame(predict(ann_model, test_ann))

names(ann_predictions)[names(ann_predictions) == "predict(ann_model, test_ann)"] <- "target"
